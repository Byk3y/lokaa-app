...
ALTER TABLE public.posts ADD COLUMN IF NOT EXISTS pin_position INTEGER;
ALTER TABLE public.posts ADD COLUMN IF NOT EXISTS pin_category TEXT;
...
CREATE INDEX IF NOT EXISTS idx_posts_pin_category ON public.posts(pin_category) WHERE is_pinned = TRUE;

-- Function to toggle post pin status with a maximum of 4 pinned posts per category
CREATE OR REPLACE FUNCTION public.toggle_post_pin(
  post_id UUID,
...
BEGIN
  ...
  SELECT is_pinned INTO is_currently_pinned FROM public.posts WHERE id = post_id;
  ...
  IF pin_action = 'unpin' THEN
    ...
    FROM numbered_posts
    WHERE public.posts.id = numbered_posts.id;
...
GRANT EXECUTE ON FUNCTION public.toggle_post_pin(UUID, TEXT, TEXT) TO authenticated;

-- Function to update pin positions
CREATE OR REPLACE FUNCTION public.update_pin_positions(
  post_ids UUID[],
  category TEXT DEFAULT 'general'
) RETURNS JSONB AS $$
DECLARE
...
-- Grant execute permission to authenticated users
GRANT EXECUTE ON FUNCTION public.update_pin_positions(UUID[], TEXT) TO authenticated; 