        ...
        <Card className="mb-6 bg-gradient-to-r from-primary/10 to-primary/5 border-primary/30 shadow-lg">
            ...
            <CardContent className="flex flex-col sm:flex-row items-center gap-4 sm:gap-6">
                ...
                <div className="flex-grow text-center sm:text-left">
                    ...
                    <p className="text-lg font-semibold text-primary">{user.levelInfo.name}</p>
                    <p className="text-sm text-muted-foreground">{user.pointsInSpace.toLocaleString()} Points</p>
                    {!user.levelInfo.isMaxLevel && (
                        <div className="mt-2">
                            <div className="flex justify-between text-xs text-muted-foreground mb-1">
                                <span>Progress to {LEVEL_THRESHOLDS.find(l => l.level === user.levelInfo.level + 1)?.name || 'Next Level'}</span>
                                <span>{user.levelInfo.pointsNeededForNextLevel.toLocaleString()} pts to go</span>
                            </div>
                            <Progress value={user.levelInfo.progressPercentage} className="h-2 bg-primary/20 [&>*]:bg-primary" />
                        </div>
                    )}
                     {user.levelInfo.isMaxLevel && (
