...
export const aboutPageSettingsSchema = z.object({
  ...
  intro_media_url: z.string().url('Invalid media URL').optional(),
  icon_image: z.string().url('Invalid icon URL').optional(),
  cover_image: z.string().url('Invalid cover URL').optional()
});
...
export const generalSettingsSchema = baseSpaceSettingsSchema.extend({
  icon_image: z.string().url('Invalid icon URL').optional(),
  cover_image: z.string().url('Invalid cover URL').optional()
});
...
export const validateIntroMedia = async (file: File, type: 'image' | 'video') => {
  const fileValidation = FileValidationService.getInstance();
  return fileValidation.validateFile(file, type);
}; 